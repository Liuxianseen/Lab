...
elseif BITS==6
    ...
    r=real(rx_denor);
    i=imag(rx_denor);
    r_deci=(r>6)*7+(r<6&r>4)*5+(r<4&r>2)*3+(r<2 &r>0)...
           +(r<0 &r>(-2))*(-1)+(r<(-2)&r>(-4))*(-3)+(r<(-4)&r>(-6))*(-5)+(r<(-6))*(-7);
    i_deci=(i>6)*7+(i<6&i>4)*5+(i<4&i>2)*3+(i<2 &i>0)...
           +(i<0 &i>(-2))*(-1)+(i<(-2)&i>(-4))*(-3)+(i<(-4)&i>(-6))*(-5)+(i<(-6))*(-7);
    rx_deci=r_deci+j*i_deci;
